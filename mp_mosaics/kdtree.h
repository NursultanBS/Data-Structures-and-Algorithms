...
class KDTree
{
  ...
  private:
     ...
     */
    double distance(const Point<Dim>& point_1, const Point<Dim>& point_2) const;
    KDTreeNode * construct_tree(vector<Point<Dim>>& list, int dimension, unsigned left, unsigned right);
    Point<Dim> quickselect(vector<Point<Dim>>& list, int dimension, unsigned left, unsigned right, unsigned z);
    ...
    void copy(KDTree<Dim> * subroot_1, KDTree<Dim> * subroot_2);
    void clear(KDTreeNode *& node);
    Point<Dim> findNearestNeighbor(const Point<Dim>& query, KDTreeNode * node, int dimension) const;
};
