...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    if (dict.find(word_1) == dict.end() || dict.find(word_2) == dict.end()) {
        return false;
    }

    if (dict.at(word_1).size() != dict.at(word_2).size()) {
        return false;
    }

    for (unsigned i = 0; i < dict.at(word_1).size(); ++i) {
        if (dict.at(word_1)[i] != dict.at(word_2)[i]) {
            return false;
        }
    }
