...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {
      ...
      if (distance <= 160) {
        pixel.l = (1 - distance * 0.005) * pixel.l;
...
PNG illinify(PNG image) {
  for (unsigned x = 0; x < image.width(); x++){
    for (unsigned y = 0; y < image.height(); y++){
      ...
      if (pixel.h < orange + (blue - orange)/2 || pixel.h > blue + ((orange + 360) - blue)/2){
        pixel.h = orange;
      } else if (pixel.h >= orange + (blue - orange)/2 && pixel.h <= blue + ((orange + 360) - blue)/2){
        pixel.h = blue;
      }
    }
  }  
